...
function displayCourse(course){
    const ele = $('<div class="course-container card-styling" id=' +
                course.courseCode +
                '><h2>'+ course.courseCode + ": " + course.courseName +
                '</h2><div class="triangle-down"></div></div>')[0];
...
function generateHeaderRow(){
    ...
    const ele = '<tr><th><h3>Assessment</h3></th><th><h3>Weight (%)</h3></th><th><h3>Mark (%)</h3></th></tr>';
...
$(document).on('click', '#add-course-container img', function(e){
    ...
    //add form 
    const form = $('<form><div><h3>Course Code: </h3>' + 
                    '<input type="text" id="course-code" /></div><br>' +
                    '<div><h3>Course Title: </h3>' +
                    '<input type="text" id="course-title" /></div><br>' +
                    '<button id="add-course-btn">Add Course</button></form>');
